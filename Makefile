...
COMMON_CFLAGS = $(WARNINGS) $(VIDEO_FLAGS) -std=c99 -march=native -I. -I../include
COMMON_CXXFLAGS = $(WARNINGS) $(VIDEO_FLAGS) -std=c++0x -march=native -I. -I../include
...
all: CFLAGS = $(COMMON_CFLAGS) $(RELEASE_CFLAGS) $(VIDEO_FLAGS)
all: $(TARGET)

debug: CFLAGS = $(COMMON_CFLAGS) $(DEBUG_CFLAGS) $(VIDEO_FLAGS)
debug: $(TARGET)

all-cpp: CFLAGS = $(COMMON_CXXFLAGS) $(RELEASE_CFLAGS) $(VIDEO_FLAGS)
all-cpp: $(TARGET)
all-cpp: CC = $(CXX)

debug-cpp: CFLAGS = $(COMMON_CXXFLAGS) $(DEBUG_CFLAGS) $(VIDEO_FLAGS)
debug-cpp: $(TARGET)
debug-cpp: CC = $(CXX)

clean:
ifeq ($(OS),windows)
	@$(ECHO) $(BLUE)Cleaning libvideo...$(TEXTRESET)
else
	@$(ECHO) "$(BLUE)Cleaning libvideo...$(TEXTRESET)"
endif
	@$(RM) $(OBJECTS) $(TARGET)
